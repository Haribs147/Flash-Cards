...
Podstawą skutecznego działania złożonych systemów jest proste diagnozowanie błędów oraz problemów wydajnościowych.
...
\begin{itemize}
    ...
    dla monitorowania modułu publicznego wyszukiwania materiałów. 
\end{itemize}

Telemetria aplikacji jest zarządzana przez centralny obiekt \texttt{TracerProvider}, który odpowiada za konfigurację 
i rejestrację procesorów. Podłączony do niego procesor wsadowy (\texttt{BatchSpanProcessor}) buforuje i przygotowywuje 
dane telemetryczne (ślady i metryki) do wysłania. Za przesył danych do serwera APM odpowiada instacja klasy 
...
łatwe zweryfikowanie wydajności aplikacji. Rysunek \ref{rys:telemetry_post} 
...
około 236 ms, co odpowiada całkowitemu czasowi obsługi zapytania. 

Co istotne, możemy zaobserwować, iż po wysłaniu 
odpowiedzi do klienta rozpoczęło się kolejne zapytanie do bazy danych, a następnie operacja w ElasticSearch.
...
\begin{enumerate}
    ...
    \item Pobraniu informacji o właścicielu zestawu (Drugi SELECT po czasie oczekiwania na API).
...
o około 2.5 sekundy. Zgodnie z oczekiwaniami najbardziej czasochłonną operacją 
w procesie generowania tagów było zapytanie do modelu Google Gemini, które zajęło aż 2.4 sekundy (96 procent całej operacji).
 
\begin{figure}
    \centering \includegraphics[width=1\linewidth]{telemetry_post_set.png} % 0.618 to złoty podział, bardzo dobra szerokość
	\caption{Uruchomione kontenery systemu w Docker Desktop}
	\label{rys:telemetry_post}
\end{figure}
