...
z serwerem API użyto biblioteki Axios. Została ona wybrana zamiast wbudowanego w JavaScript mechanizmu fetch ze względu na 
...
Dopiero po otrzymaniu odpowiedzi od serwera atrybut src jest ustawiany na adres URL otrzymany od serwera, 
...
\end{itemize}
Poniższy listing pokazuje implementację obu wyżej omówionych strategii. Funkcja \texttt{get\_db} wykorzystuje block try...finally
...
(baza danych, silnik wyszukiwania, magazyn plików itp.) postanowiono całkowicie skonteneryzować rozwiązanie i 
...
skalowalność horyzontalną. Do zarządzania kontenerami wykorzystano platformę Docker \cite{docker_docs} i 
...
Podstawą skutecznego działania złożonych systemów jest możliwość prostego diagnozowanie błędów oraz problemów wydajnościowych.
...
przedstawia sekwencję operacji na relacyjnej bazie danych PostgreSQL, które łącznie zajmują 
około 71 ms, co odpowiada całkowitemu czasowi obsługi zapytania. 

Co istotne, możemy zaobserwować, iż po wysłaniu 
odpowiedzi do klienta rozpoczął się proces generowania tagów dla zestawu fiszek.
Potwierdza to poprawność implementacji asynchronicznego przetwarzania tagów, gdzie proces ten 
wykonywany jest przy pomocy zadań w tle. Polega on na:
\begin{enumerate}
...
OpenTelemetry oprócz automatycznej instrumentacji pozwala na tworzenie własnych zakresów pomiarowych (ang. \texttt{custom spans}). 
...
Proces przesłania składa się z dwóch mierzalnych operacji:
\begin{enumerate}
    \item Przetworzenie obrazu przy pomocy biblioteki PIL (operacja CPU-bound).
...
obiekt \texttt{tracer}. Dodatkowo do śladu \texttt{pil\_image\_processing} dodano atrybut \texttt{image.size\_bytes}, który pozwala na łatwą 
...
Całkowity czas obsługi zapytania \texttt{POST/upload-image} wynosi zaledwie 12 ms.
...
udoskolanienia powinny być kierowane w stronę sieciowej infrastruktury.
...
\end{figure}
% \begin{figure}
%     \centering \includegraphics[width=1\linewidth]{telemetry_main_page.png} % 0.618 to złoty podział, bardzo dobra szerokość
% 	\caption{Uruchomione kontenery systemu w Docker Desktop}
% 	\label{rys:docker_desktop}
% \end{figure}

% \begin{figure}
%     \centering \includegraphics[width=1\linewidth]{telemetry_get_set2.png} % 0.618 to złoty podział, bardzo dobra szerokość
% 	\caption{Uruchomione kontenery systemu w Docker Desktop}
% 	\label{rys:docker_desktop}
% \end{figure}

% \begin{figure}
%     \centering \includegraphics[width=1\linewidth]{telemetry_get_set_better.png} % 0.618 to złoty podział, bardzo dobra szerokość
% 	\caption{Uruchomione kontenery systemu w Docker Desktop}
% 	\label{rys:docker_desktop}
% \end{figure}
