...
ale także właściwych standardów wytwarzania oprogramowania. Niniejszy rozdział opisuje proces implementacji poszczególnych warstw aplikacji, zaczynając 
od interfejsu użytkownika, poprzez logikę biznesową, kończąc na konteneryzacji i~telemetrii. 

Wykorzystano system kontroli wersji Git, pozwalający na zachowanie historii zmian, co umożliwiło bezpieczne eksperymentowanie i~tworzenie nowych funkcjonalności.
Cały kod źródłowy aplikacji został umieszczony w~repozytorium serwisu GitHub~\cite{githubRepo}. W~repozytorium zastosowano strukturę monorepo, co oznacza, 
...
szerokie wsparcie dla wykorzystywanych technologii oraz ogromną liczbę rozszerzeń usprawniających pracę nad kodem. 
Aby zachować spójność i~jakość kodu, skonfigurowano w~środowisku automatyczne formatowanie kodu.
Kluczowym aspektem podczas tworzenia oprogramowania jest zapewnienie czytelności i~łatwości utrzymania aplikacji, 
...
\begin{itemize}
    ...
    W~jego obrębie znajdują się podkatalogi (np. Auth lub FlashcardSet) zawierające komponenty React, które 
    stanowią bezpośrednie odzwierciedlenie projektu w~Figmie. Komponenty te odpowiadają za renderowanie interfejsu 
    i~obsługę interakcji z~użytkownikiem.
    \item \texttt{src/features} -- Katalog skupiający logikę biznesową aplikacji. Znajdują się w~nim definicje wycinków stanu
    (ang. slices) biblioteki Redux Toolkit oraz serwisy odpowiadające za komunikację z~API serwera.
\end{itemize} 
Uzupełnieniem tej struktury jest folder \texttt{src/app} zawierający globalną konfigurację magazynu stanu (ang. store) 
oraz niestandardowe hooki. Taka organizacja kodu pozwoliła na zachowanie czytelności projektu i~
...
Globalny stan obsługiwany jest za pomocą biblioteki Redux Toolkit. Pełni on funkcję głównego magazynu dla danych 
...
\begin{itemize} \sloppy
    \item \textbf{Dane sesji} -- Wycinki stanu (ang. slices) takie jak \texttt{authSlice} czy \texttt{materialsSlice} przechowują 
    informacje potrzebne do poprawnego działania interfejsu (np. informacja o~statusie zalogowania użytkownika lub 
    aktualnie otwartym folderze). Interakcje użytkownika z~systemem (np. proces logowania czy dodanie folderu) skutkują 
    natychmiastową aktualizacją stanu o~dane otrzymane w~odpowiedzi na zapytanie wysłane do serwera API.
...
Podczas wylogowywania użytkownika zostają usuwane jego ciasteczka sesyjne, dzięki czemu użytkownik nie ma dostępu do większości zapytań 
...
w momencie wylogowania (\texttt{logoutUser}), czyści globalny stan aplikacji do wartości początkowych, co skutecznie 
blokuje dostęp do chronionych zasobów użytkownika (np. jego materiałów). 

Listing \ref{lst:logout_user} pokazuje konfigurację magazynu stanu Reduxa, poprzez zaimplementowanie rootReducera.
...
w stanie oczekiwania na weryfikację tożsamości użytkownika. Jest to konieczne, ponieważ główny komponent 
aplikacji (Listing \ref{lst:app_tsx}) wstrzymuje renderowanie widoków do momentu, aż status sesji 
zmieni się z~domyślnego \texttt{"idle"}, co w~przypadku pełnego resetu stanu skutkowałoby 
wyświetleniem pustego ekranu, a~nie ekranu logowania.
...
\begin{lstlisting}[language=Python,
caption={Mechanizm blokady renderowania aplikacji do czasu ustalenia tożsamości użytkownika},
label={lst:app_tsx}]
function App() {
...
Dane te niewymagające dodatkowej logiki biznesowej po stronie klienta zarządzane są przez bibliotekę 
...
rankingi zestawów (np. "Najpopularniejsze", "Najczęściej wyświetlane"). Kluczowym elementem optymalizacji systemu
...
a dopiero po upłynięciu czasu zdefiniowanego w~\texttt{staleTime}, wysyła zapytanie do API, w~
...
z serwerem API użyto biblioteki Axios. Została ona wybrana ponad wbudowany w~JavaScript mechanizm fetch ze względu na jej
możliwości konfiguracji, wbudowaną obsługę formatu JSON oraz zaawansowany mechanizm przechwytywania żądań i~odpowiedzi
(ang. interceptor).

W celu zapewnienia spójności i~zminimalizowania redundancji kodu skonfigurowano globalne ustawienia biblioteki Axios. 
...
\begin{enumerate}
    ...
    propagację błędu do wyższych warstw aplikacji.
    \item W~celu zapobiegnięcia nieskończonym pętlom zapytań aplikacja najpierw sprawdza, czy błąd nie wystąpił w~zapytaniu do endpointów 
    obsługujących uwierzytelnianie (\texttt{/login}, \texttt{/register}, \texttt{/logout} czy \texttt{/refresh}). Błędy w~tych zapytaniach 
    zwracane są natychmiastowo, bez próby ponownego odświeżenia sesji.
    \item Jeżeli powyższe warunki są spełnione, aplikacja wysyła zapytanie do endpointu \texttt{/refresh}.
    ...
    wyloguje użytkownika, czyszcząc stan aplikacji.
\end{enumerate}
...
export const setupAxiosInterceptor = (store: AppStore) => {
    axios.interceptors.response.use(
        ...
        async (error) => {
            ...
            ... // pobranie informacji o~endpoincie 
                // oryginalnego zapytania
            if (
                error.response.status === 401 &&
                !originalRequest._retry &&
                ... // sprawdzenie czy oryginalne zapytanie 
                    // nie przyszlo z~endpointow uwierzytelniajacych 
            ) {
...
Zgodnie z~projektem systemu, edytor musi umożliwiać nie tylko 
...
Implementacja edytora opiera się na inicjalizacji instancji silnika TipTap za pomocą wbudowanego hooka 
\texttt{useEditor}. Dodatkowo skonfigurowano edytor, przekazując mu tablicę rozszerzeń, 
...
nie udostępnia interfejsu użytkownika, MenuBar obsługuje interakcję użytkownika z~silnikiem TipTap. 
Każdy przycisk znajdujący się na pasku narzędzi wywołuje sekwencję metod na obiekcie edytora za pomocą 
łańcucha poleceń. Przykładem może być pogrubienie tekstu, które wywołuje: \texttt{editor.chain().focus().toggleBold().run()}. 
Dzięki użyciu metody \texttt{focus}, 
edytor utrzymuje pole tekstowe jako aktywne, nawet po kliknięciu przycisku formatowania (np. pogrubienia), 
co znacznie usprawnia pracę użytkownika nad tekstem.
...
wstawienie obrazu do edytora tekstowego. Pozwala ono jednak tylko na wyświetlanie multimediów na podstawie 
...
mechanizm React Node View. Mechanizm ten pozwala na osadzenie interaktywnego komponentu React 
w strukturze widoku dokumentu (DOM), zastępując domyślną, statyczną reprezentację węzła obrazu.

W strukturę DOM wstrzykiwany jest autorski komponent \texttt{InteractiveImage}, który początkowo wyświetla przycisk pozwalający na wstawienie pliku. 
Plik wybrany przez użytkownika jest asynchronicznie przesyłany na serwer, który zwraca publiczny adres URL zasobu. 
Dopiero po otrzymaniu odpowiedzi od serwera atrybut \texttt{src} węzła jest aktualizowany o~zwrócony adres, 
co skutkuje natychmiastowym wyświetleniem właściwego obrazu w~miejscu przycisku.
...
frameworku FastAPI. W~przeciwieństwie do projektu systemu, gdzie sformułowano podział logiczny, w~niniejszym rozdziale
opisano organizację kodu, konfigurację serwera i~mechanizmy scalające poszczególne komponenty systemu.
...
Każdy moduł systemu, zobrazowany na diagramie funkcjonalności systemu 
...
Do przechowywania wrażliwych danych konfiguracyjnych aplikacja wykorzystuje bibliotekę pydantic-settings. 
...
przyjmowania żądań od klienta i~uruchomienie procedury zwalniania zasobów po zakończeniu działania serwera. Szczególną rolę w~tym procesie 
odgrywa zarządzanie połączeniami do poszczególnych źródeł danych. Tak jak wspomniano w~rozdziale \ref{ch:projekt_systemu} (Projekt Systemu), zainicjalizowane 
zasoby są udostępniane komponentom systemu za pomocą mechanizmu wstrzykiwania zależności. W~zależności od zasobu 
przyjęto odmienne strategie zarządzania jego cyklem życia:
\begin{itemize}
    \item {\textbf{Relacyjna baza danych}} -- Do zarządzania połączeniem z~relacyjną bazą danych zastosowano wzorzec sesji na żądanie 
    (ang. session per request). Oznacza to, że dla każdego zapytania HTTP tworzona jest nowa sesja bazy danych, która jest zamykana od razu 
    po zwróceniu wyniku do klienta, a~powiązane z~nią fizyczne połączenie jest zwracane do puli połączeń w~celu ponownego wykorzystania.
    \item {\textbf{Wyszukiwarka pełnotekstowa}} -- Wykorzystywany w~aplikacji klient Elasticsearch posiada wewnętrzne mechanizmy obsługi 
    puli połączeń HTTP. Do zarządzania jego cyklem życia zastosowano wzorzec Singleton. W~rezultacie podczas inicjalizacji 
    ...
    kosztownego narzutu tworzenia nowego klienta dla każdego żądania zapewniając efektywne zarządzanie połączeniami do realizacji poszczególnych zapytań.
\end{itemize}
Poniższy listing pokazuje implementację obu wyżej omówionych strategii. Funkcja \texttt{get\_db} wykorzystuje blok try...finally
do bezpiecznego zamknięcia połączenia niezależnie od wyniku operacji. Z~kolei funkcja \texttt{get\_es\_client} zwraca referencję do 
wcześniej zainicjalizowanego globalnego obiektu.

\begin{lstlisting}[language=Python,
	caption={Strategie zarządzania cyklem życia obiektu},
...
się na przesyłaniu tokenów JWT w~nagłówkach HTTP, zaimplementowane rozwiązanie wykorzystuje ciasteczka do przechowywania i~przesyłania tokenów uwierzytelniających.
Proces logowania i~utrzymywania sesji opiera się na dwóch kluczowych tokenach:
\begin{itemize}
    \item \textbf{Access token} -- Krótkoterminowy token dostępu (ważny przez 5 minut) pozwalający na uwierzytelnienie użytkownika.
...
W serwerze zostały zaimplementowane dwa sposoby uwierzytelniania użytkownika oparte na mechanizmie wstrzykiwania zależności:
\begin{itemize}
...
Największym wyzwaniem w~implementacji tego rozwiązania było połączenie asynchronicznej biblioteki walidującej CSRF (\texttt{fastapi\_csrf\_protect}) 
...
używa synchronicznego sterownika bazy danych i~Elasticsearcha, co doprowadziłoby do zablokowania głównej pętli zdarzeń (ang. event loop). Problem 
ten rozwiązano poprzez wykorzystanie wzorca wstrzykiwania zależności (listing \ref{lst:async_in_sync}). Walidację tokenu CSRF wydzielono do asynchronicznej funkcji, która uruchamiana 
jest asynchronicznie przed właściwą logiką endpointu. FastAPI jest w~stanie optymalnie zarządzać kontekstem wykonania: walidacja tokena uruchamia 
...
na hierarchiczną strukturę danych i~różne relacje między zasobami logika została podzielona na dwa współpracujące ze sobą serwisy: 
...
\texttt{check\_permission}, która realizuje autoryzację do poszczególnych materiałów. Przed wykonaniem jakiejkolwiek operacji na zasobie, 
...
\begin{enumerate}
    ...
    \item Następnie serwis wykorzystuje id nowo stworzonego materiału do zapisania szczegółów materiału (opis, widoczność) i~listy fiszek
...
wstawienie nowych, przychodzących z~żądania. Nie jest to jednak idealne podejście, gdyż w~przypadku małej zmiany w~dużym zestawie fiszek (np. zmiana jednej fiszki), 
...
bazie danych są aktualizowane, nieposiadające ID dodawane, a~nieistniejące w~przesłanych danych usuwane. Taki mechanizm minimalizuje liczbę operacji 
wykonywanych na bazie danych i~zachowuje spójność kluczy głównych fiszek.

Istotnym aspektem operacji zapisu do bazy danych jest zapewnienie bezpieczeństwa użytkowników, później wyświetlających zapisane dane. 
...
zostanie zapisana w~magazynie obiektowym MinIO, jest przetwarzana przez bibliotekę \texttt{Pillow}. Proces ten 
...
\begin{itemize}
    ...
    pixel flood, wszystkie grafiki przekraczające rozdzielczość FULL HD (1920x1080 pikseli) są automatycznie skalowane w~dół 
    ...
    rekonstrukcja pliku. Przy pomocy funkcji \texttt{Image.save()} z~biblioteki \texttt{Pillow} każdy obraz jest dekodowany do pamięci operacyjnej jako mapa bitowa, a~następnie zapisywany do nowego 
...
weryfikuje typ materiału. W~przypadku napotkania skrótu (\texttt{item\_type="link"}) pobiera wyżej wspomniane dane z~materiału źródłowego, przy 
...
Kluczowym aspektem optymalizacyjnym w~obsłudze tego żądania było oddelegowanie logowania wyświetlenia zestawu fiszek do zadań w~tle (\texttt{BackgroundTasks}).
BackgroundTasks jest wbudowanym mechanizmem FastAPI pozwalającym na wykonanie metody po zwróceniu odpowiedzi HTTP do użytkownika.
Dzięki temu zapis analityczny zostaje wykonywany po wysłaniu odpowiedzi do klienta, co zapobiega blokowaniu głównego wątku obsługi żądania.

System umożliwia także kopiowanie zestawów fiszek. Operacja ta pozwala użytkownikowi stworzyć głęboką kopię dowolnego zestawu fiszek, do którego 
...
powiązanych fiszek z~oryginalnego materiału do jego kopii. Dzięki temu użytkownik jest właścicielem skopiowanego zestawu, co pozwala mu na swobodną edycję i~nadawanie 
...
wyzwań implementacyjnych był wybór danych, po których działałaby wyszukiwarka. Opieranie mechanizmu wyszukiwania wyłącznie na nazwie i~opisie zestawu mogłoby 
...
Kluczowym elementem było stworzenie jasnego zapytania dla modelu (listing \ref{lst:model_prompt}), tak aby ten zwrócił jak najbardziej deterministyczną i~najtrafniejszą odpowiedź (prompt engineering). 
...
\begin{lstlisting}[language=Python,
	caption={Zapytanie dla modelu Google Gemini 2.5 Flash},
	label={lst:model_prompt}]
    prompt = f"""
...
zabezpieczenie, ograniczono zużycie modelu do 500 tokenów, aby nie zużył on zbyt dużej liczby tokenów w~przypadku błędnego zinterpretowania 
promptu.
...
generowania tagów realizowany jest w~tle (przy użyciu \texttt{BackgroundTasks}), a~samo zadanie uruchamiane jest automatycznie  
przy każdym utworzeniu lub edycji zestawu fiszek. Po zatwierdzeniu zmian w~bazie danych, zlecenie generowania tagów trafia do 
kolejki zadań, dzięki czemu serwer jest w~stanie natychmiastowo zakończyć obsługę żądania HTTP, zwracając klientowi odpowiedź o~udanym 
obsłużeniu zapytania.

Do implementacji wyszukiwania wykorzystano zapytanie Elasticsearch typu \texttt{multi\_match}, który pozwala na 
przeszukiwanie wielu pól jednocześnie. W~podstawowej konfiguracji \texttt{multi\_match} zwraca zestaw o~polu posiadającym największy wynik, 
ignorując pozostałe. Aby w~pełni wykorzystać zgromadzone dane, rozszerzono konfigurację \texttt{multi\_match} o~dwa dodatkowe mechanizmy 
rankingowania:
\begin{itemize}
    ...
    wyniku z~najbardziej dopasowanego pola dodawane są wyniki z~innych pól pomnożone przez wartość tie\_breakera.
\end{itemize}
...
zestawu fiszek. W~początkowej fazie projektowania wymagań funkcjonalnych rozważano implementację modelu komentarzy, pozwalającego na 
...
funkcjonalności (np. wcześniej wspomniana wyszukiwarka publicznych zestawów), zdecydowano się na uproszczenie logiki biznesowej modułu komentarzy do 
...
rozwój aplikacji. Z~tego powodu zaimplementowano hybrydowe podejście łączące mechanizm listy sąsiedztwa (ang. adjacency list) z~
mechanizmem zmaterializowanej ścieżki (ang. materialized path) opartym na rozszerzeniu PostgreSQL ltree. 
...
\begin{itemize}
    ...
    spowodowanego implementacją mniej wydajnych zapytań rekurencyjnych (ang. Recursive CTE), które byłyby niezbędne przy zastosowaniu podejścia 
    samej listy sąsiedztwa.
    \item \textbf{Optymalizacja usuwania komentarzy} -- Niezależnie od liczby poziomów zagnieżdżenia usunięcie rodzica zawsze musi skutkować 
    usunięciem wszystkich dzieci. Ltree pozwala na optymalizację tej operacji, poprzez wykonanie jednego zapytania SQL przy użyciu mechanizmu 
    \texttt{descendant\_of}. Eliminuje to konieczność rekursywnego usuwania całej hierarchicznej struktury drzewa, znacznie optymalizując 
    ...
    po dacie oraz zachowuje oryginalną hierarchię drzewa, co znacznie ułatwia stworzenie i~wysłanie odpowiedzi do klienta.
\end{itemize}
...
Relacyjna baza danych posiada mechanizm wyzwalacza (\texttt{trigger}), który pozwala na łatwe utrzymanie integralności danych. 
Zastosowanie wyzwalacza niesie ze sobą dwie kluczowe korzyści:
\begin{itemize}
    \item \textbf{Minimalizacja zapytań do bazy danych} -- Do wyliczenia ścieżki w~systemach używających 
    ...
    \item \textbf{Zapewnienie integralności danych} -- Wykorzystanie triggera pozwala na globalizację procesu. Oznacza to, że niezależnie 
    od tego, czy dane wprowadzone zostały przez API serwera, migracje czy bezpośrednie zapytania SQL, zmienna \texttt{path} zawsze będzie zaktualizowana.
...
\section{Konteneryzacja i~telemetria}

Ostatnim, lecz kluczowym etapem było przygotowanie środowiska wykonawczego i~monitorowania aplikacji. 
Z uwagi na złożoność architektury, składającej się z~dużej liczby komunikujących się ze sobą usług 
(baza danych, silnik wyszukiwania, magazyn plików i~serwer), postanowiono całkowicie skonteneryzować rozwiązanie i~
wdrożyć zaawansowaną telemetrię.

\subsection{Konteneryzacja środowiska}
Skonteneryzowanie zapewnia zachowanie spójności środowiska uruchomieniowego oraz znacznie ułatwia późniejszą 
skalowalność horyzontalną systemu. Do zarządzania kontenerami wykorzystano platformę Docker \cite{docker_docs} oraz 
narzędzie Docker Compose do ich definiowania oraz uruchamiania. Plik konfiguracyjny (\texttt{docker-compose.yml}) 
definiuje pełny stos aplikacji, zgodny z~zaprojektowaną architekturą przedstawioną na rysunku \ref{rys:architecture_diagram}.

Poprawność konfiguracji i~uruchomienia wszystkich serwisów zweryfikowano przy użyciu aplikacji Docker Desktop, co 
przedstawia rysunek \ref{rys:docker_desktop}.

Widoczne na rysunku \ref{rys:docker_desktop} kontenery odpowiadają poszczególnym warstwom systemu:
\begin{itemize}
    \item \textbf{flashcards-frontend} -- Aplikacja kliencka dostępna na porcie 5173.
    \item \textbf{flashcards-backend} -- Serwer API aplikacji działający na porcie 8000.
    \item \textbf{my-postgres-db i~elasticsearch} -- Baza danych i~silnik wyszukiwania. Komunikują się one z~serwerem 
    wewnątrz wyizolowanej sieci Docker, wystawiając swoje porty jedynie dla celów administracyjnych.
    \item \textbf{minio-container} -- Magazyn obiektów, udostępniający dwa porty: 9000 (obsługujący API serwera) oraz 
    9001 (interfejs graficzny administratora). 
    \item \textbf{apm-server i~kibana} -- Narzędzia do telemetrii. APM Server odpowiada za gromadzenie śladów i~metryk 
    aplikacji, a~Kibana udostępnia interfejs graficzny do ich wizualizacji.
\end{itemize}

\begin{figure}[h]
    \centering \includegraphics[width=0.85\linewidth]{docker_desktop.png} % 0.618 to złoty podział, bardzo dobra szerokość
	\caption{Uruchomione kontenery systemu w~Docker Desktop}
	\label{rys:docker_desktop}
\end{figure}

Wykorzystanie Docker Compose znacznie ułatwiło testowanie i~wdrażanie aplikacji. Dzięki niemu komunikacja 
sieciowa między kontenerami odbywa się automatycznie po nazwach serwisów, co eliminuje konieczność ręcznej konfiguracji adresów IP.

\subsection{Telemetria}
Podstawą skutecznego działania złożonych systemów jest możliwość prostego diagnozowania błędów oraz problemów wydajnościowych.
Logowanie samych zdarzeń często okazuje się niewystarczające, 
dlatego zdecydowano się na wprowadzenie mechanizmu dokładnego monitorowania aplikacji.
Aby to zapewnić, zaimplementowano mechanizm telemetrii oparty na standardzie OpenTelemetry. Pozwala on na generowanie, zbieranie 
i eksportowanie śladów (ang. traces), metryk oraz logów niezależnie od narzędzia wykorzystywanego do ich analityki, co pozytywnie wpływa 
na elastyczność programowanej aplikacji.

Implementacja telemetrii dla serwera API opiera się na wykorzystaniu mechanizmu auto-instrumentacji, który pozwala na 
automatyczne przechwytywanie zdarzeń z~głównych bibliotek bez konieczności dodatkowej modyfikacji kodu logiki biznesowej.
W systemie skonfigurowano trzy kluczowe instrumentatory:
\begin{itemize}
    \item \textbf{FastAPIInstrumentor} -- Jest odpowiedzialny za śledzenie przychodzących zapytań HTTP, mierzenie czasu ich 
    wykonania, przechwytywanie kodów odpowiedzi i~błędów na serwerze. 
    \item \textbf{SQLAlchemyInstrumentor} -- Nadzoruje zapytania do relacyjnej bazy danych (PostgreSQL), co pozwala na dokładną 
    analizę czasu wykonywania poszczególnych zapytań SQL.
    \item \textbf{ElasticsearchInstrumentor} -- Monitoruje zapytania do wyszukiwarki pełnotekstowej (Elasticsearch), co jest kluczowe 
    do monitorowania modułu publicznego wyszukiwania materiałów. 
\end{itemize}

Telemetria aplikacji jest zarządzana przez centralny obiekt \texttt{TracerProvider}, który odpowiada za konfigurację 
i rejestrację procesorów. Podłączony do niego procesor wsadowy (\texttt{BatchSpanProcessor}) buforuje i~przygotowuje 
dane telemetryczne (ślady i~metryki) do wysłania. Za przesył danych do serwera APM odpowiada instancja klasy 
\texttt{OTLPSpanExporter}, która przesyła dane przy użyciu protokołu OTLP.
Cały proces konfiguracji telemetrii został przedstawiony na listingu \ref{lst:opentelemetry_config}.

\begin{lstlisting}[language=python, 
caption={Konfiguracja OpenTelemetry}, 
label={lst:opentelemetry_config}]
...
Zaimplementowanie tak szczegółowego monitorowania serwera pozwoliło na 
łatwe zweryfikowanie wydajności aplikacji. Rysunek \ref{rys:telemetry_post_set} 
przedstawia ślad zapytania odpowiedzialnego za utworzenie nowego zestawu 
fiszek (\texttt{POST /sets}). Wykres ten 
wizualizuje sekwencję operacji na relacyjnej bazie danych PostgreSQL, które łącznie zajmują 
około 71 ms, co odpowiada całkowitemu czasowi obsługi zapytania. 

Warto zauważyć, że dopiero po wysłaniu 
odpowiedzi do klienta serwer rozpoczął proces generowania tagów dla zestawu fiszek.
Potwierdza to poprawność implementacji asynchronicznego przetwarzania tagów, 
realizowanego przy pomocy zadań w~tle. Polega on na:
\begin{enumerate}
    \item Pobraniu zawartości zestawu fiszek (Pierwszy SELECT od razu po odesłaniu odpowiedzi do klienta).
    \item Wysłaniu zapytania do zewnętrznego API Google Gemini w~celu wygenerowania tagów.
    \item Pobraniu informacji o~właścicielu zestawu.
    \item Zapisaniu danych w~Elasticsearch.
\end{enumerate}
Dzięki przeniesieniu tego procesu do zadań w~tle czas oczekiwania użytkownika na odpowiedź został skrócony 
o około 2,51 sekundy. Zgodnie z~oczekiwaniami, najbardziej czasochłonną operacją 
w procesie generowania tagów było zapytanie do modelu Google Gemini, które zajęło aż 99,56 procent całej operacji.
 
\begin{figure}[h]
    \centering \includegraphics[width=1\linewidth]{telemetry_post_set_with_gemini.png} % 0.618 to złoty podział, bardzo dobra szerokość
	\caption{Ślad procesu tworzenia zestawu fiszek}
	\label{rys:telemetry_post_set}
\end{figure}

OpenTelemetry oprócz automatycznej instrumentacji pozwala także na tworzenie własnych zakresów pomiarowych (ang. \texttt{custom spans}). 
Dzięki nim możemy w~łatwy sposób uzyskać informacje o~czasie wykonywania zapytań do zewnętrznych API, co zostało przedstawione 
na rysunku \ref{rys:telemetry_post_set} (ślad \texttt{gemini\_generate\_tags}). Niestandardowe ślady przydatne są 
także w~przypadku serwisów, które nie mają zaimplementowanej automatycznej instrumentacji lub operacji obciążających procesor (CPU-bound).
Zostały one także zastosowane w~module obsługi multimediów \texttt{MediaService}, odpowiedzialnym za przesyłanie i~przetwarzanie 
obrazów użytkownika. 

Do zaimplementowania niestandardowych śladów widocznych na rysunku \ref{rys:telemetry_post_set} i~\ref{rys:telemetry_upload_file} wykorzystano 
obiekt \texttt{tracer} z~biblioteki OpenTelemetry. Umożliwił on zaimplementowanie ręcznej instrumentacji polegającej na prostym objęciu kluczowych 
bloków logiki biznesowej menedżerem kontekstu \texttt{start\_as\_current\_span}. Dodatkowo do śladu \texttt{pil\_image\_processing} dodano atrybut \texttt{image.size\_bytes}, który pozwala na łatwą 
analizę korelacji wielkości pliku z~czasem jego przetwarzania. 

Proces przesyłania multimediów składa się z~dwóch mierzalnych operacji:
\begin{enumerate}
    \item Przetworzenie obrazu przy pomocy biblioteki \texttt{Pillow} (operacja CPU-bound).
    \item Wysłanie przetworzonego pliku do magazynu danych Minio (operacja I/O-bound).
\end{enumerate}

Całkowity czas obsługi zapytania \texttt{POST /upload-image} widocznego na rysunku \ref{rys:telemetry_upload_file} wynosi zaledwie 12 ms.
Wykres wyraźnie ukazuje dysproporcję między czasem operacji CPU-bound (span \texttt{pil\_image\_processing} -- około 1,1 ms) oraz I/O-bound
(span \texttt{upload\_to\_minio} -- około 5,8 ms). Dzięki temu można stwierdzić, że większa optymalizacja kodu Python nie przyniesie dużych korzyści, a~potencjalne 
udoskonalenia powinny być kierowane w~stronę sieciowej infrastruktury.

\begin{figure}[h]
    \centering \includegraphics[width=1\linewidth]{telemetry_upload_file.png} % 0.618 to złoty podział, bardzo dobra szerokość
	\caption{Ślad procesu obsługi multimediów}
	\label{rys:telemetry_upload_file}
\end{figure}

% Implementację manulanych spanów wraz z~dodaniem atrybytu \texttt{image.size\_bytes} przedstawia listing \ref{lst:manual_instrumentation}.

% \begin{lstlisting}[language=python, 
% caption={Implementacja niestandardowych śladów w~serwisie multimediów}, 
% label={lst:manual_instrumentation}]
