...
Z tego powodu podjęto decyzję o pogrupowaniu powiązanych ze sobą funkcji w siedem głównych modułów. Taki podział 
...
\subsubsection*{Wymagania funkcjonalne:}
\begin{itemize}
    \item Użytkownik może udostępnić swój zestaw fiszek innemu zarejestrowanemu użytkownikowi. 
...
\begin{itemize}
    ...
    \item System zezwala na zagnieżdżanie komentarzy do najwyżej jednego poziomu.
...
Wymagania niefunkcjonalne definiują standardy i cechy, 
jakie system musi spełniać, aby działać skutecznie i wydajnie \cite{sommerville2015}. 
Sprecyzowanie poniższych wymagań niefunkcjonalnych było
...
\begin{itemize}
    ...
    \item \textbf{Autoryzacja:} System musi kontrolować dostęp użytkowników do zasobów. Użytkownik niebędący właścicielem materiału lub niemający do niego uprawnień nie powinien uzyskać do niego dostępu.
    ...
    \item \textbf{Walidacja plików:} System musi walidować wszystkie pliki przesyłane przez użytkowników pod kątem rozmiaru, typu i możliwie zagnieżdżonego złośliwego kodu.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Czas odpowiedzi wyszukiwania:} Czas odpowiedzi wyszukiwania publicznych materiałów powinien średnio wynosić mniej niż dwie sekundy.
    ...
    \item \textbf{Wydajność zapytań:} System musi efektywnie pobierać złożone hierarchiczne struktury danych, aby nie powodować nadmiernego obciążania bazy danych.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Edytor fiszek:} System musi zapewniać intuicyjny i łatwy w użyciu edytor tekstu (WYSIWYG) do tworzenia fiszek umożliwiający zaawansowane formatowanie tekstu i dodawanie obrazków.
    ...
    \item \textbf{Przepływ pracy:} Aplikacja musi wspierać płynny przepływ pracy użytkownika poprzez automatyczne przekierowania np. po udanym zalogowaniu.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Skalowalność horyzontalna:} Serwer musi być bezstanowy (ang. stateless), aby umożliwić uruchomienie jego wielu instancji i rozdzielenie ruchu sieciowego (ang. load balancing).
    \item \textbf{Separacja komponentów:} Kluczowe komponenty (np. logika aplikacji, serwis wyszukiwania) powinny być od siebie oddzielone, aby umożliwić ich niezależne skalowanie w przyszłości.
...
W przypadku zaakceptowania udostępnienia, jego status w material\_shares zostanie zmieniony na zaakceptowany, a w encji materials pojawia się nowy wpis 
...
a ten zapisuje obraz w dedykowanym kontenerze danych (ang. bucket). Link do każdego obrazka jest zapisywany w relacyjnej bazie danych, bezpośrednio 
w tekście fiszki. Kontener ma ustawione publiczne uprawnienia odczytu, więc klient nie potrzebuje żadnych dodatkowych uprawnień do 
wyświetlenia obrazka, co znacznie upraszcza implementację.
