...
Podstawowe badania z zakresu psychologii poznawczej wyjaśniają, dlaczego stosowanie fiszek jest tak wydajną metodą nauki.
Wskazują one na kluczową różnicę między biernym a aktywnym przyswajaniem wiedzy. Tradycyjne bierne metody dydaktyczne, polegające na 
...
jego przeczytaniu \cite{brown2014make}. Cała przyswojona wiedza w rzeczywistości jednak często pozostaje tylko w pamięci krótkotrwałej, czyniąc ją ulotną.
...
pamięci krótkotrwałej. Podejście to polega na dwóch kluczowych mechanizmach kognitywnych:
\begin{itemize}
    ...
    materiału, co udowodniono w eksperymentach Roedigera i Karpickego \cite{roediger2006test}.
...
\begin{itemize}
    ...
    \item \textbf{Wady:} Wszystkie wersje aplikacji (desktop, mobile i web) charakteryzują się bardzo nieintuicyjnym i przestarzałym interfejsem graficznym. 
...
\subsection*{Quizlet}
Quizlet to komercyjne rozwiązanie działające w modelu freemium, które zdobyło popularność dzięki intuicyjnemu interfejsowi 
i dostępności z poziomu przeglądarki oraz aplikacji mobilnej. Firma kładzie większy nacisk na implementację różnorodnych 
form nauki (np. poprzez gry) niż na surowe algorytmy powtórek.
\begin{itemize}
    ...
    \item \textbf{Wady:} Ograniczony edytor tworzenia fiszek, pozwalający jedynie na podstawowe formatowanie (zakreślanie, kursywa, pogrubienie) oraz 
    uniemożliwiający wstawianie wielu zdjęć do jednej fiszki. System zarządzania materiałami jest także mało elastyczny, ponieważ nie pozwala na 
    tworzenie zagnieżdżonych struktur folderów, co znacząco utrudnia porządkowanie wiedzy.
\end{itemize}

\subsection*{Brainscape}
Brainscape to rozwiązanie działające w modelu freemium, dostępne jako aplikacja mobilna oraz webowa. Wykorzystuje on zaawansowaną 
metodę metapoznania i samooceny do przyśpieszenia procesu nauki. 
W przeciwieństwie do standardowych algorytmów, w których użytkownik dokonuje wyboru odrzucenia/zostawienia fiszki, tutaj musi on 
samodzielnie ocenić stopień znajomości odpowiedzi w skali od 1 do 5, co bezpośrednio wpływa na czas do następnej powtórki.
\begin{itemize}
    ...
    nastawiona na szybkie tworzenie prostych fiszek, a edytor treści pozwala na wstawianie plików dźwiękowych i graficznych. Oferuje także możliwość 
    generowania fiszek za pomocą sztucznej inteligencji oraz importowanie danych bezpośrednio z pliku. 
    \item \textbf{Wady:} Ograniczone możliwości edytora treści fiszek, który pozwala na zaawansowane formatowanie jedynie przy wykorzystaniu języka 
    Markdown. System organizacji wiedzy nie pozwala na zagnieżdżanie folderów (określanych w aplikacji mianem klas), brakuje również funkcjonalności 
    oceniania cudzych zestawów fiszek. 
\end{itemize}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{} p{2.2cm} X X X X @{}}
        ...
        Klasy i talie (Płaska struktura) & 
        \textbf{Pełny eksplorator plików (Zagnieżdżone foldery)} \\ 
        \addlinespace
        
        \textbf{Edytor treści} & 
        HTML/CSS do zaawansowanego formatowania & 
        Uproszczony & 
        Markdown do zaawansowanego formatowania & 
        \textbf{Zaawansowany i prosty w obsłudze} \\ 
        \addlinespace
